...
# Triage short-circuit event is optional too. Prefer the 3-category event
# (coordinator_shortcircuit_event); fall back to the older greeting-only event
# if the streaming module hasn't been updated yet.
try:
    from streaming.events import coordinator_shortcircuit_event
    SHORTCIRCUIT_EVENT_AVAILABLE = True
except ImportError:
    SHORTCIRCUIT_EVENT_AVAILABLE = False
try:
...
def coordinator_node(state: BlackboardState) -> Dict[str, Any]:
    ...
    if workflow_step == WorkflowStep.INITIAL:
        ...
        # =====================================================================
        # TOP-LAYER TRIAGE — fast 3-way query classification + short-circuit.
        # Single small-model LLM call (~200-500ms). Categories:
        #   - "greeting" → social pleasantry; reply with friendly invite, END.
        #   - "general"  → answerable from general/meta knowledge (no agents
        #                  needed); reply with the LLM's direct answer, END.
        #   - "academic" → run the full MAS pipeline (resolver, classifier,
        #                  agents, evaluation, synthesis).
        # Failure mode is safe: any error → category="academic", reply="" →
        # full pipeline runs as usual. False negatives never block users.
        # =====================================================================
        triage_start = time.time()
        triage = coordinator.triage_query(user_query)
        triage_time = time.time() - triage_start
        category = triage.get("category", "academic")

        if category in ("greeting", "general"):
            # Reply was generated by the triage LLM (or filled with a default).
            reply = triage.get("reply") or (
                coordinator._DEFAULT_GREETING_REPLY if category == "greeting"
                else coordinator._DEFAULT_GENERAL_REPLY
            )
            ...
            if STREAMING_AVAILABLE and SHORTCIRCUIT_EVENT_AVAILABLE:
                try:
                    emit_event(coordinator_shortcircuit_event(
                        category=category, reply=reply, triage_time=triage_time
                    ))
                except Exception:
                    pass
            elif STREAMING_AVAILABLE and GREETING_EVENT_AVAILABLE and category == "greeting":
                # Backward-compat: older streaming module only knew greetings.
                try:
